...
func TestGetUserWithoutAvatarByName(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT * FROM `imgs` WHERE user_id = ? AND is_avatar = ? ORDER BY `imgs`.`id` LIMIT ?")).
		WithArgs(user.ID, true, 1).
		WillReturnRows(sqlmock.NewRows([]string{"id", "name", "user_id", "is_avatar", "created_at"}))

	u, avatar, err := repo.GetUserByNameWithAvatar(user.Username)
	// 因为确实没有头像
	assert.Error(t, err, gorm.ErrRecordNotFound)
	assert.Empty(t, avatar)
	assert.Equal(t, user.Username, u.Username)
	u.CheckPassword(user.Pwd)
}
...
func TestGetUserWithAvatarByName(t *testing.T) {
	...
	user_rows := sqlmock.NewRows([]string{
		`id`, `username`, `email`, `pwd`, `bio`,
		"created_at", "updated_at", "last_activity",
		"failed_login", "captcha_req_cnt",
	}).AddRow(
	...
	// 构造头像响应
	img_rows := sqlmock.NewRows([]string{
		"id", "name", "user_id", "is_avatar", "created_at",
	}).AddRow(
		img.ID, img.Name, user.ID, img.IsAvatar, img.CreatedAt,
	)
...
func TestGetUserWithoutAvatarById(t *testing.T) {
	...
	u, avatar, err := repo.GetUserByIdWithAvatar(user.ID)
	// 因为确实没有头像
	assert.Error(t, err, gorm.ErrRecordNotFound)
	assert.Empty(t, avatar)
	assert.Equal(t, user.Username, u.Username)
